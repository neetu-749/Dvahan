...
function ContractContextProvider(props) {
    ...
    const Services = {
        registerManufacturer: async (_name) => {
            try {
                if (!state.Vahan) return { success: true, data: {} }

                const manufacturerRegistrationResponse = await state.Vahan.methods.addManufacturer(_name).send({
                    from: account,
                    gas: Constants.GAS
                })
        ...
        registerRetailer: async (_name) => {
            try {
                ...
                const retailerRegistrationResponse = await state.Vahan.methods.addRetailer(_name).send({
                    from: account,
                    gas: Constants.GAS
                })
        ...
        registerCustomer: async (_name) => {
            try {
                ...
                const customerRegistrationResponse = await state.Vahan.methods.addCustomer(_name).send({
                    from: account,
                    gas: Constants.GAS
                })
        ...
        getManufacturer: async (_address) => {
            try {
                ...
                const manufacturer = await state.Vahan.methods.getManufacturerDetails(_address).call()
        ...
        getRetailer: async (_address) => {
            try {
                ...
                const retailer = await state.Vahan.methods.getRetailerDetails(_address).call()
        ...
        getCustomer: async (_address) => {
            try {
                ...
                const customer = await state.Vahan.methods.getCustomerDetails(_address).call()
        ...
        getManufacturerInventory: async (_address) => {
            try {
                ...
                const productIDList = await state.Vahan.methods.getManufacturerInventory(_address).call()

                let products = []
                for await (let id of productIDList) {
                    const product = await state.Vahan.methods.products(parseInt(id)).call()
        ...
        getRetailerInventory: async (_address) => {
            try {
                ...
                const productIDList = await state.Vahan.methods.getRetailerInventory(_address).call()
        ...
        getCustomerOrders: async (_address) => {
            try {
                ...
                const productIDList = await state.Vahan.methods.getCustomerOrders(_address).call()
        ...
        addProduct: async (_name, _price, _ipfs_hash) => {
            try {
                if (!state.Vahan) return { success: true, data: {} }
                const date = Date.now()
                const addProductResponse = await state.Vahan.methods.addProduct(_name, _price, _ipfs_hash, date).send({
                    from: account,
                    gas: Constants.GAS
                })
        ...
        getProduct: async (_productId) => {
            try {
                ...
                const productResponse = await state.Vahan.methods.getProductDetails(_productId).call()
        ...
        getCurrentStatus: async (_productId) => {
            try {
                ...
                const currentStatusResponse = await state.Vahan.methods.getCurrentStatus(_productId).call()
                return {
                    success: true, data: {
        ...
        getUserType: async (_address) => {
            try {
                ...
                const userType = await state.Vahan.methods.getUserType(_address).call()
        ...
        getProductHistory: async (_productId) => {
            try {
                ...
                const allEvents = await state.Vahan.getPastEvents('allEvents', {
                    fromBlock: 0,
                    toBlock: 'latest'
                })
        ...
        getAllProducts: async () => {
            try {
                if (!state.Vahan) return { success: true, data: {} }
                const allProducts = await state.Vahan.methods.getProducts().call()
        ...
        releaseProduct: async (_productId) => {
            try {
                if (!state.Vahan) return { success: true, data: {} }
                const date = Date.now()
                const product = await state.Vahan.methods.products(_productId).call()

                const releaseProductResponse = await state.Vahan.methods.releaseProduct(_productId, date).send({
                    from: account,
                    gas: Constants.GAS
                })
        ...
        buyProduct: async (_productId) => {
            try {
                ...
                const product = await state.Vahan.methods.products(_productId).call()
                const currentRetailerAddress = await state.Vahan.methods.Product_Retailers(_productId, product.total_retailers - 1).call()
                // const currentRetailerAddress = retailers[retailers.length - 1]

                const buyProductResponse = await state.Vahan.methods.buyProduct(_productId, date).send({
                    from: account,
                    gas: Constants.GAS
                })
